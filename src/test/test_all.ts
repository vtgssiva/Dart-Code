...
async function runAllTests(): Promise<void> {
	if (process.env.CI) {
		const branchName = process.env.APPVEYOR_REPO_BRANCH || process.env.TRAVIS_BRANCH || process.env.CIRRUS_BRANCH;
		const commit = process.env.APPVEYOR_REPO_COMMIT || process.env.TRAVIS_COMMIT || process.env.CIRRUS_CHANGE_IN_REPO;
