...
import { AnalyzerStatusNotification } from "./custom_protocol";

export class LspAnalyzerStatusReporter {
	private analysisInProgress = false;
	private analyzingPromise?: PromiseCompleter<void>;

	constructor(readonly analyzer: LanguageClient) {
		analyzer.onReady().then(() => {
			analyzer.onNotification(AnalyzerStatusNotification.type, (params) => this.handleServerStatus(params.isAnalyzing));
		});
	}
